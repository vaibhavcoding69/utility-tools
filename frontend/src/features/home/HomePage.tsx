...
const categoryFilters = [
  { label: 'Developer', caption: 'Encoding, payloads, diffing', href: '/tools/developer' },
  { label: 'Security', caption: 'Passwords, hashes, TOTP', href: '/tools/security' },
  { label: 'Data', caption: 'CSV/JSON, SQL, fake data', href: '/tools/data' },
]

const toolIndex = [
  { name: 'JSON Formatter', description: 'Format & validate JSON payloads', href: '/tools/developer/json', category: 'Developer', keywords: ['json', 'format', 'validate'] },
  { name: 'Base64 Encoder', description: 'Encode/decode Base64 strings', href: '/tools/developer/base64', category: 'Developer', keywords: ['base64', 'encode', 'decode'] },
  { name: 'URL Encoder', description: 'Escape URLs safely', href: '/tools/developer/url', category: 'Developer', keywords: ['url', 'encode', 'decode'] },
  { name: 'UUID Generator', description: 'Generate UUID v1 & v4', href: '/tools/developer/uuid', category: 'Developer', keywords: ['uuid', 'id', 'unique'] },
  { name: 'Regex Tester', description: 'Run expressions with flags', href: '/tools/developer/regex', category: 'Developer', keywords: ['regex', 'pattern', 'test'] },
  { name: 'JWT Decoder', description: 'Inspect token payloads', href: '/tools/developer/jwt', category: 'Developer', keywords: ['jwt', 'token'] },
  { name: 'Text Diff', description: 'Compare two inputs', href: '/tools/developer/diff', category: 'Developer', keywords: ['diff', 'compare'] },
  { name: 'Password Generator', description: 'Create secure passwords', href: '/tools/security/password', category: 'Security', keywords: ['password', 'generator'] },
  { name: 'Hash Generator', description: 'SHA, MD5, and more', href: '/tools/security/hash', category: 'Security', keywords: ['hash', 'sha', 'md5'] },
  { name: 'TOTP Generator', description: '2FA compatible codes', href: '/tools/security/totp', category: 'Security', keywords: ['totp', '2fa'] },
  { name: 'CSV to JSON', description: 'Convert CSV quickly', href: '/tools/data/csv-to-json', category: 'Data', keywords: ['csv', 'json'] },
  { name: 'JSON to CSV', description: 'Flatten arrays to CSV', href: '/tools/data/json-to-csv', category: 'Data', keywords: ['json', 'csv'] },
  { name: 'SQL Formatter', description: 'Beautify SQL queries', href: '/tools/data/sql', category: 'Data', keywords: ['sql', 'format'] },
  { name: 'Fake Data', description: 'Generate personas & more', href: '/tools/data/fake-data', category: 'Data', keywords: ['fake', 'data', 'mock'] },
  { name: 'Base Converter', description: 'Convert between bases', href: '/tools/data/base-converter', category: 'Data', keywords: ['base', 'convert'] },
]
...
function UtilityDock() {
  ...
  return (
    <section className="command-dock" aria-label="Tool navigation">
        ...
        <div className="command-results" role="listbox">
          {visibleResults.length ? (
            visibleResults.map((tool) => (
              <Link key={tool.name} to={tool.href} className="command-result-item">
                <div>
                  <p>{tool.name}</p>
      ...
      <div className="command-pills" role="navigation" aria-label="Filter categories">
        {categoryFilters.map((category) => (
          <Link key={category.label} className="command-pill" to={category.href}>
      ...
      <div className="quick-actions-grid">
        {quickActions.map((action) => (
          <Link key={action.name} to={action.href} className="quick-action-card">
  ...
  return (
    <section className="section">
      ...
      <div className="stats-grid">
        {stats.map((stat) => (
          <div key={stat.label} className="stats-card reveal" style={{ transitionDelay: `${stats.indexOf(stat) * 90}ms` }}>
