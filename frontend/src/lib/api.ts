...
class ApiClient {
  ...
  async generateUuid(version: number = 4, count: number = 1) {
    return this.request<ApiResponse>(`/developer/uuid/generate?version=${version}&count=${count}`);
  }
  ...
  async generatePassword(options: {
    ...
    if (options.include_uppercase !== undefined) params.append('include_uppercase', String(options.include_uppercase));
    if (options.include_lowercase !== undefined) params.append('include_lowercase', String(options.include_lowercase));
    if (options.include_numbers !== undefined) params.append('include_numbers', String(options.include_numbers));
    if (options.include_symbols !== undefined) params.append('include_symbols', String(options.include_symbols));
    if (options.exclude_ambiguous !== undefined) params.append('exclude_ambiguous', String(options.exclude_ambiguous));
    if (options.count) params.append('count', String(options.count));
    
    return this.request<ApiResponse>(`/security/password/generate?${params.toString()}`);
  }
  ...
  async checkPasswordPolicy(password: string, policy: {
    min_length?: number;
  ...
  async generateHash(data: string, algorithm: string = 'sha256', encoding: string = 'hex') {
  ...
  async generateApiKey(prefix: string = 'sk', length: number = 32) {
    return this.request<ApiResponse>(`/security/secret/api-key?prefix=${prefix}&length=${length}`);
  }
  ...
  async generateFakeData(dataType: string, count: number = 10, locale: string = 'en_US') {
